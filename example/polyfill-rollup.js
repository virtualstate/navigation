...
async function getImportUUIDOrNodeRandomUUID() {
    const { v4 } = await Promise.resolve().then(function () { return importUuid; })
        ...
        .catch(async () => Promise.resolve().then(function () { return _rollup_plugin_ignore_empty_module_placeholder$1; }))
        .catch(() => undefined)
        .then((mod) => mod ?? fakeUUID);
    return { v4 };
}

const getUuidModule = () => importUuid;
...
const v4 = GlobalUUID; //await importUUID();
